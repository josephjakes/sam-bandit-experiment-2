...
jsPsych.plugins['survey-multi-choice'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // Move submit button to bottom if only a single question
    var submitLocation = "";
    if (trial.questions.length == 1) {
      submitLocation = "@media screen and (min-height: 700px) {input[type='submit'] { position: absolute; bottom: 100px margin-left: -60px}";
    }
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      // add question text
      html += '<p class="jspsych-survey-multi-choice-text survey-multi-choice bold">' + question.prompt 
    ...
    // add submit button
    html += '<input type="submit" id="'+plugin_id_name+'-next" class="'+plugin_id_name+' jspsych-btn btn btn-outline-primary"' + (trial.button_label ? ' value="'+trial.button_label + '"': '') + '></input>';
    html += '</form>';
